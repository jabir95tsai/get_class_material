...
- **`--version` / `-V`** reads `ntu_cool_materials.__version__`, which is derived from installed package metadata (`importlib.metadata`) — pyproject.toml is the single source of truth, never hardcode a version literal. `pick_main` intercepts `--version` before forwarding to the `pick` subparser.
- **Update check** (`update_check.check_for_update`) runs on every `pick` (in a background thread overlapping `ensure_ready`, so zero added latency) and in `doctor`'s header. It hits PyPI's JSON API at most once per 24h (cached in `<secrets>/update_check.json`), compares via a lenient numeric-tuple parse, and is **silent on any error** (offline, parse failure) — a startup nag must never block or crash. Returns `None` for the `0.0.0+unknown` source-tree version so devs aren't nagged.
