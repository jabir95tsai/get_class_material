...
4. `download_youtube` — yt-dlp subprocess per week with rename pass.
...
- **Secrets stay in `.secrets/`** (git-ignored). `materials/`, `ntu-cool-gcm_material/`, `*.sqlite3`, `*.m4s`, `*.part`, `.env` are also ignored. Never write cookies, tokens, or downloaded course files outside these paths.
